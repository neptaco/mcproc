...
impl DaemonClient {
    pub async fn connect(socket_path: Option<PathBuf>) -> Result<Self, Box<dyn std::error::Error>> {
        ...
        #[cfg(unix)]
        {
            ...
            if let Err(e) = Self::check_and_restart_if_needed(&mut daemon_client, &socket_path).await {
    ...
    async fn check_and_restart_if_needed(client: &mut Self, _socket_path: &PathBuf) -> Result<(), Box<dyn std::error::Error>> {
        let request = proto::GetDaemonStatusRequest {};
        match client.inner().get_daemon_status(request).await {
            Ok(response) => {
                let status = response.into_inner();
                if status.version != VERSION {
                    eprintln!("Daemon version mismatch: daemon={}, client={}", status.version, VERSION);
