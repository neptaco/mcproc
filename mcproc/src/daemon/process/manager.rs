...
impl ProcessManager {
    ...
    pub async fn start_process_with_log_stream(
        ...
        let log_file = self.config.log.dir.join(format!(
            "{}_{}.log",
        ...
        tokio::spawn(async move {
            ...
            let timeout_future = if has_pattern_stdout {
                let duration =
                    tokio::time::Duration::from_secs(wait_timeout_stdout.unwrap_or(30) as u64);
        ...
        tokio::spawn(async move {
            ...
            let timeout_future = if has_pattern_stderr {
                let duration =
                    tokio::time::Duration::from_secs(wait_timeout_stderr.unwrap_or(30) as u64);
                tokio::time::sleep(duration)
            } else {
                tokio::time::sleep(tokio::time::Duration::from_secs(u64::MAX)) // Never timeout
            };
        ...
        if let Some((_, rx)) = log_ready_tx {
            let timeout_duration =
                tokio::time::Duration::from_secs(wait_timeout.unwrap_or(30) as u64);
            ...
            debug!(
                "No wait_for_log pattern, will check health after 500ms for process {}",
                name
            );
    ...
    pub async fn restart_process(
            ...
            self.get_process_by_name_or_id_with_project(name_or_id, project.as_deref())
        {
            ...
            tokio::time::sleep(tokio::time::Duration::from_millis(
                self.config.process.restart_delay_ms,
            ))
            .await;
