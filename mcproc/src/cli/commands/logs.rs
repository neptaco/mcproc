            ...
            None => {
                ...
                if !self.follow {
                    ...
                    all_entries.sort_by(|a, b| {
                        let a_ts = a.timestamp.as_ref().map(|t| (t.seconds, t.nanos)).unwrap_or((0, 0));
                        let b_ts = b.timestamp.as_ref().map(|t| (t.seconds, t.nanos)).unwrap_or((0, 0));
                        a_ts.cmp(&b_ts)
                    });
                    ...
                    let max_name_len = processes.iter()
                        .map(|p| p.name.len())
                        .max()
                        .unwrap_or(10)
                        .max(10); // Minimum 10 characters
                    
                    stream_multiple_logs(client, processes, project, self.tail, max_name_len).await?;
                }
            }
        }
...
fn print_log_entry_with_process_padded(entry: &proto::LogEntry, max_name_len: usize) {
    ...
    let colored_padded_name = match process_name.chars().fold(0u8, |acc, c| acc.wrapping_add(c as u8)) % 5 {
        0 => padded_name.green(),
        1 => padded_name.blue(),
        2 => padded_name.cyan(),
...
async fn stream_multiple_logs(
    ...
    if monitored_processes.is_empty() {
        match &project {
            Some(p) => {
                eprintln!("{} Waiting for processes to start in project: {}", 
                    "→".yellow(), 
                    p.cyan().bold()
                );
            }
            None => {
                eprintln!("{} Waiting for processes to start (no project context)", 
                    "→".yellow()
                );
            }
        }
                ...
                Ok(response) => {
                    ...
                    for process in current_processes {
                        if !local_monitored.contains(&process.id) {
                            eprintln!("{} New process started: {}", "→".green(), process.name.green().bold());
                            ...
                            tokio::spawn(async move {
                                ...
                                match client_clone.inner().get_logs(request).await {
                                    ...
                                    Err(e) => {
                                        eprintln!("{} Error streaming logs for {}: {}", "✗".red(), process_name, e);
                                    }
                                }
                            });
